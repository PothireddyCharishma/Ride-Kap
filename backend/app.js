...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    myCloud = {
      public_id: "https://asset.cloudinary.com/duqy1baw3/3823cfd540a736c33eaf2b5555ff576f",
      secure_url: "https://res.cloudinary.com/duqy1baw3/image/upload/v1714415910/avatars/Profile_ymxnex.png"
    }
...
app.get("/profile", async (req, res) => {
  ...
  if (token && token !== "j:null") {
    jwt.verify(token, jwtSecret, {}, async (err, userData) => {
      if (err) throw err;
      const { name, email, phone, _id, avatar } = await User.findById(userData.id);
      res.json({
        name,
        email,
        phone,
